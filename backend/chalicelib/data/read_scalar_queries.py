...
@use_db_session()
def get_room_song_id_from_room_song_guid(room_song_guid: str, db_session: Session) -> Optional[int]:
    return db_session.query(RoomSong.room_song_id).filter(RoomSong.room_song_guid == room_song_guid).scalar()


@use_db_session()
def get_user_id_from_username(username: str, db_session: Session) -> Optional[int]:
    return db_session.query(User.user_id).filter(User.user_username == username).scalar()


@use_db_session()
def get_room_guid_from_username(username: str, db_session: Session) -> Optional[int]:
...
@use_db_session()
def get_room_id_from_room_guid(room_guid: str, db_session: Session) -> Optional[int]:
