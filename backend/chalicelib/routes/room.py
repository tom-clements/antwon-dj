...
@room_routes.route("/room", methods=["GET"], cors=get_cors_config())
...
@room_routes.route("/room", methods=["DELETE"], cors=get_cors_config(), authorizer=get_authorizer())
...
@room_routes.route("/roomQueue", methods=["GET"], cors=get_cors_config())
...
@room_routes.route("/roomQueue", methods=["POST"], cors=get_cors_config())
def room_queue_post():
...
@room_routes.route("/room/queue", methods=["DELETE"], cors=get_cors_config(), authorizer=get_authorizer())
def room_songs_purge_post():
    params = room_routes.current_request.query_params
    delete_queue(params["room_guid"])


@room_routes.route("/room/song/like", methods=["POST"], cors=get_cors_config(), authorizer=get_authorizer())
