...
def test_read_room_queue_guest(mock_read_room_queue_guest: Mock, queue: QueueResult, is_room_exist: bool) -> None:
    ...
    actual_queue = get_room_queue_guest_from_room_guid.__wrapped__(room_guid)  # type: ignore
...
def test_read_room_queue_user(
    ...
    actual_queue = get_room_queue_user_from_room_guid.__wrapped__(username, room_guid)  # type: ignore
    assert actual_queue == [q.db_result for q in queue.sorted_songs]
