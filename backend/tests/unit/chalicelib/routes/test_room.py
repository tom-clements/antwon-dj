...
def test_room_queue_get(mock_get_room_queue_from_room_guid: Mock, test_client: Client) -> None:
    room_guid = "test_room_guid"
    queue = [{"song": "test"}]
    expected = {"room_queue": queue}
    mock_get_room_queue_from_room_guid.return_value = queue
    response = test_client.http.get(f"/room/{room_guid}/queue")
