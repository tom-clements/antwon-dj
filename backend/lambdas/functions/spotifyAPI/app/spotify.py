...
def get_spotify_secrets():
    secrets = app.get_secret('spotify_client')
    return secrets

def app_Authorization():
    secrets = get_spotify_secrets()
    f = {'response_type': 'code', 'client_id': secrets['SPOTIFY_CLIENT_ID'], 'scope': SCOPES, 'redirect_uri': REDIRECT_URI}#, state: state}
...
def get_token(code):
    secrets = get_spotify_secrets()
    data = {'code': code, 'redirect_uri': REDIRECT_URI, 'grant_type': 'authorization_code'}#, state: state}
    url = 'https://accounts.spotify.com/api/token'
    base64encoded = base64.b64encode(bytes("{}:{}".format(secrets['SPOTIFY_CLIENT_ID'], secrets['SPOTIFY_CLIENT_SECRET']), 'utf-8'))
...
def get_spotify_session(f):
    ...
    def decorated(*args, **kwargs):
        user_id = app.db_queries.get_room_id_from_guid(kwargs['room_guid'])
        spotify_user = app.db_queries.get_spotify_user_details(user_id)
        try:
            ...
            try:
                ...
                app.db_queries.update_access_token(spotify_user['spotify_user_id'], token)
