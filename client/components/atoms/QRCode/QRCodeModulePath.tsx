...
export function getSquareModulePath(i: number, j: number, moduleSize: number): QRCodeModulePath {
    const transformX = Math.round(i * moduleSize);
    const transformY = Math.round(j * moduleSize);
    const moduleWidth = Math.round((i + 1) * moduleSize) - transformX;
    const moduleHeight = Math.round((j + 1) * moduleSize) - transformY;
    return {
